...
@Component({
  selector: 'app-countbymotherregistration',
  standalone: true,
  imports: [
    FormsModule,
    MatCard,
  ...
  templateUrl: './countbymotherregistration.component.html',
  styleUrl: './countbymotherregistration.component.scss'
})
export class CountbymotherregistrationComponent implements OnInit{

  countbymotherreg!: CountByMotherRegistration[];
  data!: MatTableDataSource<CountByMotherRegistration>;
  ...
  loadTable(query:string){
    this.rs.getMotherRegistrationCount(query).subscribe({
      next:data => {
        this.countbymotherreg = data;
        //console.log(this.countbyPorders);
        this.loadCharts();
        this.data = new MatTableDataSource(this.countbymotherreg);
      }
    });
  }
  ...
  drawCharts() {
    ...
    this.countbymotherreg.forEach((po: CountByMotherRegistration) => {
    ...
    const pieOptions = {
      title: 'Mothers Count  (Pie Chart)',
      height: 400,
      width: 500
    };

    const lineOptions = {
      title: 'Mothers Count  (Line Chart)',
      height: 400,
      width: 550
    };
